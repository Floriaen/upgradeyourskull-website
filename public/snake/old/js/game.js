...
function loadGame() {
		...
		for (var i = 0; i < col; i++) {
			...
			for (var j = 0; j < row; j++) {
				...
				if (j == 5 && i == 5) {
					...
					// keep the same ground during all the game 
					var e = i * col + j;
		...
		Score = {
			...
			createBoard: function() {
				var it = Crafty.e("2D, DOM, Text")
					...
					.css({"fontFamily": 'SilkscreenNormal', "fontSize": '60px', "text-align": "center", "color": "white"})
					.text("0");
				
				var foodEaten = function(data) {
					it.text(itemGenerator.itemGenerated);
	...
	Crafty.scene("loading", function() {
		...
		Crafty.e("2D, DOM, Text").attr({x: 0, y: 154, w: WIDTH * 2, h: 20})
			.css({fontFamily: 'SilkscreenNormal', fontSize: '33px', "text-align": "center", "color": "white"})
			.text("Loading");
	});
	...
	Crafty.scene("menu", function() { 
		...
		Crafty.e("2D, DOM, Text").attr({x: 0, y: 84, w: WIDTH * 2, h: 20})
			...
			.css({fontFamily: 'SilkscreenNormal', fontSize: '50px', "text-align": "center", "color": "white"})
			.text(l + "NAKE");
		...
		Crafty.e("2D, DOM, Text").attr({x: 0, y: 300, w: WIDTH * 2, h: 20})
			.css({fontFamily: 'SilkscreenNormal', fontSize: '24px', "text-align": "center", "color": "white"})
		...
		Crafty.e("2D, DOM, Text").attr({x: 0, y: 360, w: WIDTH * 2, h: 20})
			...
			.text("-<br /><h4>How to play?</h4><p>Control the worm <br />and find as much items as possible<br /> by infecting the carrots</p>");
	...
	Crafty.scene("main", function() {
		...
		Crafty.e("2D, DOM, Color, Tween, Alert")
			...
			.bind("TweenEnd", function(e) {
				...
				StatisticsSender.getInstance().send(itemGenerator.itemGenerated, foodGenerator.foodGenerated, Math.round(startTime), Math.round(endTime));
			})
			.bind("EnterFrame", function() {
				if (this.display()) {
					this.displayed();
		...
		Crafty.c('SnakeHead', {
			...
			enrage: function() {
				this.enrageStartFrame = Crafty.frame();
				this._isEnraged = true;
			},

			calm: function() {
				this.enrageStartFrame = null;
				...
				//	.collision(new Crafty.polygon([[2, 2],[30, 2],[30, 30],[2, 30]]))
					.bind("EnterFrame", function() {
						...
						if (toPoint) {
							...
							// var l = this.y / TILE_SIZE;
							// this.z = ZINDEX.tail + 100 + l;
						}	
					})
		...
		snake = Crafty.e("2D, Canvas, Moveable, SnakeHead, SpriteAnimation, Collision, snake")
			...
			.bind("EnterFrame", function(e) {	
				...
				if (this._stop == false) {
					...
					// copy because the PF modify the map
					var map = [];
					...
					if (this.toPoint[0] == 0 || goal) {
						...
						if (worm.isInvisible() || worm.isInvicible()) {
							...
							if (worm.isStoppedSinceTooLong()) {
								this.enrage();
							} 
							
							if ((Crafty.frame() - this.enrageStartFrame > 40) || ((worm.x == 5 * TILE_SIZE && worm.y == 5 * TILE_SIZE))) {
								this.calm();
							}
							...
							if (this.isEnraged()) {
								// speed and kill!
								this.speed += oldSpeed + 1; // tween ?
								if (this.speed > Speed.SNAKE_SPEED * 5) {
									this.speed = Speed.SNAKE_SPEED * 5;
								}
							} else						
						...
						if (this.path && this.path.length > 0) {
						    ...
						    if (nextPoint) {
								...
								if (this.positions.length == 5) { // the limit
									// var el = this.positions.shift();
									// 								wipe(el);
									// 								for (var i = 0; i < this.tail.length; i++) {
									// 									console.log("B " + this.tail[i].startIndex);
									// 									this.tail[i].startIndex--; // update all the startIndex
									// 									console.log("A " + this.tail[i].startIndex);
									// 								}
									//debugger;
									...
									// force garbage, wipe all in the array
									for (var i = 0; i < this.positions.length; i++) {
										wipe(this.positions[i]);
									}
									this.positions.length = 0; // reset the Array
