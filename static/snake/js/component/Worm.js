...
Crafty.c('Worm', {
	...
	isStoppedSinceTooLong: function() {
		return (this._stopAtFrame && ((Crafty.frame() - this._stopAtFrame) >= 50));
	},
	...
	move: function() {
		...
		if (this.x === this.toX && this.y === this.toY) {
			...
			if (Crafty.keydown.ArrowUp && Crafty.keydown.ArrowUp === true) {

				this.toY -= TILE_SIZE;
				this._stopAtFrame = null;
			} else
			if (Crafty.keydown.ArrowDown && Crafty.keydown.ArrowDown === true) {
				this.toY += TILE_SIZE;
				this._stopAtFrame = null;
			} else {
				// init the stop
